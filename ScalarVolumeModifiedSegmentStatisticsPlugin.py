...
class ScalarVolumeModifiedSegmentStatisticsPlugin(SegmentStatisticsPluginBase):
  """Statistical plugin for segmentations with scalar volumes"""

  def __init__(self):
    super(ScalarVolumeModifiedSegmentStatisticsPlugin,self).__init__()
    self.name = "Scalar Volume Modified"
  ...
  def computeStatistics(self, segmentID):
    ...
    count = vtk.vtkImageAccumulate()
    count.SetInputData(grayscaleNode.GetImageData())
    count.SetStencilData(stencil.GetOutput())
    count.Update()
    ...
    if "voxel_count" in requestedKeys:
      stats["voxel_count"] = count.GetVoxelCount()
    if "volume_mm3" in requestedKeys:
      stats["volume_mm3"] = count.GetVoxelCount() * cubicMMPerVoxel
    if "volume_cm3" in requestedKeys:
      stats["volume_cm3"] = count.GetVoxelCount() * cubicMMPerVoxel * ccPerCubicMM
    if count.GetVoxelCount()>0:
      if "min" in requestedKeys:
        stats["min"] = stat.GetMinimum()
      if "max" in requestedKeys:
        stats["max"] = stat.GetMaximum()
      if "mean" in requestedKeys:
        stats["mean"] = stat.GetMean()
      if "median" in requestedKeys:
        stats["median"] = stat.GetMedian()
      if "stdev" in requestedKeys:
        stats["stdev"] = stat.GetStandardDeviation()
